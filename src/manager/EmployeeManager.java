...
public class EmployeeManager {
    ...
    public UserDetail getUserDetail(String userId) {
        try {
            PreparedStatement statement= DatabaseConnection.getInstance().getConnection().prepareStatement("SELECT userId,username,CAST(AES_DECRYPT(password,'ramindu') AS CHAR(100)),role FROM `User Detail` WHERE userId=?");
    ...
    private boolean isUserDetailAdded(Employee e, Connection connection) {
        try {
            ...
            PreparedStatement statement=connection.prepareStatement("INSERT INTO `User Detail` VALUES(?,?,AES_ENCRYPT(?,'ramindu'),?)");
